...
const LiveSystemPreview = () => {
  ...
  const SkillRow = ({ skill, delay }: { skill: any; delay: number }) => {
  const ref = useRef(null);
  const inView = useInView(ref, { once: true });

  return (
    <div ref={ref} className="flex items-center gap-3">
      <span className="text-[11px] text-white/40 font-mono tracking-wide w-28 truncate">
        {skill.name}
      </span>
        ...
        {Array.from({ length: skill.max }).map((_, i) => (
          <motion.div
            ...
            style={{
              ...
              boxShadow:
                i < skill.level ? `0 0 6px ${skill.color}55` : "none",
            }}
          />
        ))}
      </div>

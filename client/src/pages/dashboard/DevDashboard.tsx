...
// ─── Scoring engine (pure — stays on frontend) ───────────────────────────────
...
// ─── Backend fetch — single call through our axios instance ──────────────────
    ...
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="flex flex-col items-center gap-0.5 relative z-10">
        <span className="font-black leading-none"
          style={{ fontSize: 52, letterSpacing: "-0.05em", color: meta.color, fontFamily: "'DM Mono', monospace", textShadow: `0 0 32px ${meta.glow}` }}>
          {tier}
        </span>
...
function MiniGraph({ weeks }: { weeks: ContribWeek[] }) {
  ...
  return (
    <div className="overflow-x-auto" style={{ WebkitOverflowScrolling: "touch" }}>
      <svg width={weeks.length * STEP} height={7 * STEP} style={{ display: "block" }}>
        {weeks.map((week, wi) =>
          week.contributionDays.map((day, di) => {
            const pct = day.contributionCount / maxCount;
            return (
              <rect key={`${wi}-${di}`}
                x={wi * STEP} y={di * STEP} width={CELL} height={CELL} rx={2}
                fill="#6366f1" opacity={pct === 0 ? 0.08 : 0.2 + pct * 0.8}
              />
            );
          })
        )}
...
function ConnectForm({ onSubmit }: { onSubmit: (u: string) => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1] }} className="max-w-md mx-auto">
      <div className="rounded-2xl p-8 flex flex-col items-center text-center gap-6 relative overflow-hidden"
        style={{ background: t.isDark ? "rgba(255,255,255,0.03)" : t.card, border: `1px solid ${t.border}` }}>
        ...
        <div className="relative w-16 h-16 rounded-2xl flex items-center justify-center mx-auto"
          style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.25)", boxShadow: "0 0 32px rgba(99,102,241,0.15)" }}>
        ...
        <form onSubmit={handleSubmit} className="w-full relative flex flex-col gap-3">
          <div className="flex items-center gap-2.5 px-4 py-3 rounded-xl"
            ...
            <input type="text" placeholder="github-username" value={value}
              onChange={(e) => setValue(e.target.value)}
              className="flex-1 bg-transparent outline-none text-[13px] placeholder:opacity-30"
              style={{ color: t.textPrimary, fontFamily: "'DM Mono', monospace" }} autoFocus />
          </div>
          <button type="submit" disabled={!value.trim()}
            ...
            style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", color: "#fff", fontFamily: "'DM Mono', monospace", boxShadow: "0 4px 24px rgba(99,102,241,0.3)" }}>
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <div className="text-center">
        ...
        <p className="text-[11px]" style={{ color: t.textFaint, fontFamily: "'DM Mono', monospace" }}>{steps[step]}…</p>
      </div>
...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <button onClick={onRetry} className="flex items-center gap-2 px-4 py-2 rounded-xl text-[11px]"
    ...
    <div className="space-y-5">
      ...
      <motion.div {...stagger(0)} className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <img src={data.avatarUrl} alt={data.username} className="w-9 h-9 rounded-xl object-cover"
          ...
          <div className="ml-1 flex items-center gap-1 px-2.5 py-1 rounded-full"
      ...
      {/* Rank card */}
      <motion.div {...stagger(1)} className="relative rounded-2xl overflow-hidden"
        style={{
          background: t.isDark ? "linear-gradient(145deg, rgba(255,255,255,0.04), rgba(255,255,255,0.01))" : t.card,
        ...
        <div className="relative p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-8">
            ...
            <div className="flex flex-col items-center gap-3 shrink-0">
              ...
              <div className="text-center">
                <p className="text-[10px] uppercase tracking-[0.12em] mb-0.5"
                  style={{ color: "rgba(255,255,255,0.3)", fontFamily: "'DM Mono', monospace" }}>Final Score</p>
            ...
            <div className="flex-1 w-full min-w-0">
              <p className="text-[10px] uppercase tracking-[0.1em] mb-4"
                style={{ color: "rgba(255,255,255,0.3)", fontFamily: "'DM Mono', monospace" }}>Score Breakdown</p>
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[10px] font-bold tabular-nums"
                        style={{ color: nextM.color, fontFamily: "'DM Mono', monospace" }}>{score.pctToNext}%</span>
                    </div>
                  </div>
                  <div className="h-1.5 w-full rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.06)" }}>
                    <motion.div className="h-full rounded-full"
                      initial={{ width: 0 }} animate={{ width: `${score.pctToNext}%` }}
                      transition={{ duration: 1.2, delay: 0.8, ease: [0.16, 1, 0.3, 1] }}
                      style={{ background: `linear-gradient(90deg, ${meta.color}, ${nextM.color})`, boxShadow: `0 0 8px ${nextM.color}60` }}
                    />
                  </div>
                </div>
              ) : (
      ...
      <motion.div {...stagger(2)} className="flex items-center justify-center gap-1 flex-wrap">
        {TIER_ORDER.slice().reverse().map((tier) => {
          ...
          return (
            <div key={tier} className="flex flex-col items-center gap-1 px-3 py-2 rounded-xl transition-all"
              style={{
      ...
      {data.contribWeeks.length > 0 && (
        <motion.div {...stagger(3)} className="rounded-2xl overflow-hidden"
          ...
          <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: `1px solid ${t.border}` }}>
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] px-1.5 py-0.5 rounded"
                style={{ background: "rgba(99,102,241,0.1)", color: "#818cf8", fontFamily: "'DM Mono', monospace", border: "1px solid rgba(99,102,241,0.2)" }}>
                Last 52 weeks
              </span>
            </div>
          ...
          <div className="px-5 py-4"><MiniGraph weeks={data.contribWeeks} /></div>
        </motion.div>
      )}
      ...
      {data.topLanguages.length > 0 && (
        <motion.div {...stagger(5)}>
          ...
          <div className="rounded-2xl p-5" style={{ background: t.isDark ? "rgba(255,255,255,0.025)" : t.card, border: `1px solid ${t.border}` }}>
        ...
        <motion.div {...stagger(6)}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {data.pinnedRepos.map((repo) => (
              <div key={repo.name} className="rounded-2xl p-4 flex flex-col gap-2"
                style={{ background: t.isDark ? "rgba(255,255,255,0.025)" : t.card, border: `1px solid ${t.border}` }}>
                <div className="flex items-start justify-between gap-2">
                  <p className="text-[12px] font-semibold truncate" style={{ color: t.textPrimary, letterSpacing: "-0.01em" }}>{repo.name}</p>
                ...
                {repo.desc && <p className="text-[10px] leading-relaxed line-clamp-2" style={{ color: t.textFaint }}>{repo.desc}</p>}
...
export default function DevDashboard() {
  ...
  const load = useCallback(async (u: string) => {
    ...
    try {
      ...
      // Axios wraps backend error bodies under err.response.data
  ...
  return (
    <div className="min-h-screen p-4 md:p-6 lg:p-8 transition-colors duration-300"
      ...
      >
        ...
        <div className="ml-auto text-[9px] px-2.5 py-1 rounded-full uppercase tracking-widest"
        ...
        {loading ? (
          ...
          <motion.div key="error" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            ...
            <div className="mt-4 flex justify-center">
              <button onClick={handleReset} className="text-[11px]"

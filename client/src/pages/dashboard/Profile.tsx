      ...
      >
        ...
        <div
          ...
          style={{
            ...
            background: badge.earned ? r.bg : t.cardAlt,
            border: `1px solid ${badge.earned ? r.border : t.border}`,
            boxShadow: badge.earned ? `0 0 28px ${r.glow}` : "none",
            opacity: badge.earned ? 1 : 0.38,
          }}
        >
          {!badge.earned && <Lock size={9} className="absolute top-2.5 right-2.5" style={{ color: t.textFaint }} />}
          {badge.earned && badge.rarity === "legendary" && (
            <Sparkles size={9} className="absolute top-2.5 right-2.5" style={{ color: r.text }} />
          )}
          <div
            className="w-11 h-11 rounded-xl flex items-center justify-center"
            style={{ background: badge.earned ? `${r.text}1a` : t.mutedBtn, color: badge.earned ? r.text : t.textFaint }}
          >
            {badge.icon}
          </div>
          <div className="text-center">
            <p className="text-[11px] font-semibold leading-tight" style={{ color: t.textSecondary }}>{badge.label}</p>
            {badge.earned && (
              <p className="text-[9px] font-bold uppercase tracking-widest mt-1" style={{ color: r.text, fontFamily: "'DM Mono', monospace" }}>
                {r.label}
              </p>
            )}
          </div>
        ...
        <div
          ...
          style={{
            ...
            background: badge.earned ? `${r.text}0d` : t.cardAlt,
            border: `1px solid ${badge.earned ? r.border : t.border}`,
          }}
        >
          <p className="text-[11px] font-semibold leading-tight" style={{ color: badge.earned ? r.text : t.textFaint }}>
            {badge.label}
          </p>
          <p className="text-[10px] leading-relaxed" style={{ color: t.textMuted }}>{badge.desc}</p>
          {!badge.earned && (
            <span
              className="text-[9px] px-2 py-0.5 rounded-full mt-1"
              style={{ background: t.mutedBtn, color: t.textFaint, fontFamily: "'DM Mono', monospace" }}
            >
              LOCKED
            </span>
          )}
...
function Section({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  ...
  return (
    <div
      className={`rounded-2xl p-5 transition-colors ${className}`}
...
function SectionLabel({ icon, children }: { icon?: React.ReactNode; children: React.ReactNode }) {
  ...
  return (
    <div className="flex items-center gap-2 mb-5">
      ...
      <p className="text-[11px] uppercase tracking-[0.08em]" style={{ color: t.textFaint, fontFamily: "'DM Mono', monospace" }}>
        {children}
      </p>
    </div>
  );
      ...
      <div className="max-w-4xl mx-auto space-y-4">
        ...
        >
          <div
            ...
            style={{
              background: [
                "radial-gradient(ellipse 60% 80% at 10% -10%, rgba(99,102,241,0.10) 0%, transparent 60%)",
                "radial-gradient(ellipse 40% 60% at 90% 110%, rgba(139,92,246,0.06) 0%, transparent 60%)",
              ].join(","),
            }}
          />
          <div
            className="absolute inset-0 pointer-events-none"
            style={{ backgroundImage: "radial-gradient(circle, currentColor 1px, transparent 1px)", backgroundSize: "24px 24px", opacity: t.isDark ? 0.04 : 0.03, color: t.isDark ? "#fff" : "#000" }}
          />
            ...
            <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
              ...
              >
                <div
                  ...
                  style={{ background: "rgba(99,102,241,0.1)", border: "1px solid rgba(99,102,241,0.22)", boxShadow: "0 0 40px rgba(99,102,241,0.14)" }}
                >
                  {char.icon}
                </div>
                <div
                  className="absolute -bottom-2 -right-2 px-2 py-0.5 rounded-lg text-[10px] font-bold text-white"
                  style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", fontFamily: "'DM Mono', monospace", boxShadow: "0 4px 12px rgba(99,102,241,0.4)" }}
                >
                  LV {level}
                </div>
              </motion.div>
              ...
              <div className="flex-1 min-w-0 text-center sm:text-left">
                <motion.p {...fadeUp(0.08)} className="text-[11px] uppercase tracking-widest mb-1" style={{ color: t.textMuted, fontFamily: "'DM Mono', monospace" }}>
                  {char.tagline}
                </motion.p>
                <motion.h1 {...fadeUp(0.1)} className="text-[28px] font-bold mb-0.5" style={{ color: t.textPrimary, letterSpacing: "-0.025em" }}>
                  {username}
                </motion.h1>
                <motion.p {...fadeUp(0.12)} className="text-[15px] font-semibold mb-5" style={{ color: "#818cf8" }}>
                  {char.title}
                </motion.p>
                ...
                <motion.div {...fadeUp(0.14)} className="max-w-sm">
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <span className="text-[10px] font-medium" style={{ color: "#818cf8", fontFamily: "'DM Mono', monospace" }}>
                      {currentXP.toLocaleString()} / {totalXP.toLocaleString()} XP · {xpPct}%
                    </span>
                  </div>
                  <div className="h-2 rounded-full overflow-hidden w-full" style={{ background: t.inputBg }}>
                    <motion.div
        ...
        <motion.div {...fadeUp(0.1)} className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <motion.div
              ...
              style={{ background: t.card, border: `1px solid ${t.border}` }}
            >
              <div className="w-9 h-9 rounded-xl flex items-center justify-center shrink-0" style={{ background: `${s.color}16`, color: s.color }}>
                {s.icon}
              </div>
              <div className="min-w-0">
                <p className="text-[19px] font-bold leading-none" style={{ color: t.textPrimary, letterSpacing: "-0.02em" }}>{s.value}</p>
                <p className="text-[10px] mt-0.5" style={{ color: t.textFaint, fontFamily: "'DM Mono', monospace" }}>{s.label}</p>
                <p className="text-[9px] mt-0.5 font-semibold" style={{ color: s.color, fontFamily: "'DM Mono', monospace" }}>{s.sub}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
            ...
            <Section className="h-full">
              ...
              <div className="space-y-4">
                {[
                  ...
                  return (
                    <motion.div key={d.label} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 + i * 0.07 }}>
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-[10px] font-bold px-2 py-0.5 rounded-md" style={{ color: d.color, background: d.bg, fontFamily: "'DM Mono', monospace" }}>
                          {d.label}
                        </span>
                        <span className="text-[11px] tabular-nums" style={{ color: t.textMuted, fontFamily: "'DM Mono', monospace" }}>
                          {d.count} / {d.total}
                        </span>
                      </div>
                ...
                <div className="mt-2 pt-4 flex items-center justify-between" style={{ borderTop: `1px solid ${t.border}` }}>
                  <span className="text-[11px]" style={{ color: t.textFaint, fontFamily: "'DM Mono', monospace" }}>Total completed</span>
                  <span className="text-[13px] font-semibold" style={{ color: t.textPrimary, fontFamily: "'DM Mono', monospace" }}>97 tasks</span>
                </div>
              </div>
            </Section>
          </motion.div>
            ...
            <Section className="h-full">
              ...
              <div className="flex flex-col">
                {RECENT_ACTIVITY.map((item, i) => (
                  ...
                  >
                    <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0" style={{ background: `${item.color}18`, color: item.color }}>
                      {item.icon}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] truncate" style={{ color: t.textSecondary, fontFamily: "'DM Sans', sans-serif" }}>{item.text}</p>
                      <p className="text-[10px] mt-0.5" style={{ color: t.textFaint, fontFamily: "'DM Mono', monospace" }}>{item.sub}</p>
                    </div>
                    {item.xp && (
                      <span className="text-[10px] font-semibold shrink-0" style={{ color: "#818cf8", fontFamily: "'DM Mono', monospace" }}>
                        +{item.xp} XP
                      </span>
                    )}
                  </motion.div>
